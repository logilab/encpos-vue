<template>
  <article class="documentation">
    ...
    <div class="grid-container layout-main">
      ...
      <section class="main container content">
        <section class="section" id="documentation-top">
          ...
          <p>
            ...
            chartes est ouverte en Libre accès (<em>Open Access</em>). Elle est construite
            sur différentes API qui standardisent l’accès à toutes les ressources
            disponibles (textes et images) et en font une véritable plateforme d’accès aux
            données ouvertes (<em>Open Data</em>).
          </p>
          <p>
            Cette application entend répondre aux exigences éditoriales de l’accès libre et
            aux attentes scientifiques combinées de l’ouverture des données.
          </p>
          <p>
          ...
          <ul>
            <li>
              Rendre les données FAIR (Findable, Accessible, Interoperable and Reusable).
            </li>
        ...
        <section class="section" id="ids-pattern">
          ...
          <p>
            ...
            identifiants signifiants définis pour ce projet reflètent simplement cette
            structure :
          </p>
          <ul>
            <li><code>ENCPOS</code> : la collection racine</li>
            <li>
              <code>ENCPOS_AAAA</code> : la collection des positions des thèses de l’année
              {AAAA}
            </li>
        ...
        <section class="section" id="dts-api">
          ...
          <p>
            La spécification <em>Distributed Text Services</em> (DTS) standardise l’accès
            aux collections et aux textes grâce à différents points d’entrée
            (<em>endpoints</em>).
          </p>
        </section>
        <section class="section" id="dts-collections">
          ...
          <p>
            ...
            >
            permet de parcourir les collections et d’accéder aux métadonnées d’une position.
          </p>
          <pre>{{ DTS_URL }}/navigation?id={id}</pre>
          <p>
            Une collection contient les métadonnées de la collection ainsi que le tableau de
            ses membres. Un membre de la collection peut être une collection (ressource de
            type <code>Collection</code>), ou un document (sa description, ressource de type
            <code>Resource</code>).
          </p>
          ...
          <ul>
            <li>
              <code>dc:title</code> : titre (et son code langue ISO 639-2 dans une liste)
            </li>
            <li>
              <code>dc:publisher</code> : éditeur (label et URI de référence dans une liste)
            </li>
            <li>
          ...
          <ul>
            ...
            <li>
              ...
              <code>Navigation</code> pour la ressource, décrivant le modèle documentaire
              (un document = une position dans notre cas).
            </li>
            <li>
              <code>dts:download</code> : lien de téléchargement du fichier source XML/TEI
            </li>
          </ul>
          <p><code>dts:dublincore</code> liste des métadonnées Dublin Core :</p>
          <ul>
            ...
            <li>
              <code>dc:creator</code> : auteur de la position (dans une liste, son label et
              les URI disponibles pour le liage : Sudoc, data.bnf.fr, Wikidata, Wikipedia,
              DBpedia)
            </li>
            <li>
              <code>dc:date</code> : date de soutenance de la thèse et de publication de la
              position
            </li>
            <li>
              <code>dc:language</code> : liste des langues utilisées (code
            ...
            <li>
              <code>dc:extent</code> : pagination de la version imprimée -&gt; basculer dans
              <code>dc:source</code> ?
            </li>
            <li>
              <code>dc:publisher</code> : éditeur (label et URI de référence dans une liste)
            ...
            <li>
              <code>dc:isVersionOf</code> : liste des URI Sudoc, HAL, BENC et/ou ThENC@ de
              la thèse résumée par la position
            </li>
            <li>
              <code>dc:source</code> : référence et URI de la source imprimée, et lien aux
              ressources IIIF
            </li>
          </ul>
          <div class="api-calls">
        ...
        <section class="section" id="dts-document">
          ...
          <p>
            La réponse est un fichier XML/TEI P5, conforme aux schémas maintenus par l’École
            des chartes. Voir la documentation.
          </p>
          <p>
            Le paramètre de requête <code>format</code> permet de spécifier un autre format
            de réponse. À ce stade, seul le format <code>html</code> est implémenté.
          </p>
          <div class="api-calls">
            <api-call-dropdown
              method="GET"
          ...
          <p>
            ...
            <a href="https://github.com/chartes/encpos" target="_blank">projet Github</a>
        ...
        <section class="section" id="iiif-presentation">
          ...
          <p>
            ...
            métadonnées sont donc inscrites de manière à être lues par un humain, mais sont
            structurées sémantiquement pour être disponibles pour une machine.
          </p>
          <ul>
            <li>
          ...
          <p>
            Les ressources IIIF (de type <code>Manifest</code> et <code>Collection</code>)
            sont aussi accessibles depuis la page d’accueil du
        ...
        <section class="section" id="iiif-image">
          ...
          <p>
            ...
            spécifie un service web qui renvoie une image en réponse à une demande HTTPS.
            L’URI peut spécifier la région, la taille, la rotation, les caractéristiques de
            qualité et le format de l’image demandée. L’API IIIF Image favorise ainsi la
  ...
  {scheme}://{server}{/prefix}/{identifier}/{region}/{size}/{rotation}/{quality}.{format}</pre
          ...
          <ul>
            ...
            <li>
              ...
              <code>ENCPOS_AAAA_NN_PP.TIF</code> (<code>PP</code> pour le numéro d’ordre des
              pages).
            </li>
          </ul>
          <p>
          ...
          <p>
            L’API IIIF Image spécifie aussi un URI pour fournir des informations techniques
            sur l’image, telles que ses dimensions (<code>height</code> et
        ...
        <section class="section" id="dts-iiif-links">
          ...
          <p>
            ...
            <a
              ...
              target="_blank"
              >Cookbook DTS</a
            >, un lien (<code>dts:dublicore.dc:source</code>) pointe vers la ressource IIIF
            correspondante, à chaque niveau de la structure éditoriale du corpus.
            Réciproquement, dans les ressources IIIF, un lien (<code>seeAlso</code>) pointe
            vers la réponse DTS correspondante.
          </p>
          <ul>
            <li>
              ...
              <code><a :href="`${IIIF_URL}/collection/top`" target="_blank">top</a></code>
              IIIF.
            </li>
            <li>
