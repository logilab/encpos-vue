...
export default {
  ...
  setup(props) {
    ...
    const onScroll = function() {
      if( !scrolling ) {
        scrolling = true;
        (!window.requestAnimationFrame) ? setTimeout(updateButton, 250) : window.requestAnimationFrame(updateButton);
      }
    };
    ...
    const updateButton = function() {
      ...
      const minScrollForButton = (documentHeight - windowHeight) / 2;
    ...
    const backToTop = function (event) {
      ...
      (!window.requestAnimationFrame) ? window.scrollTo(0, 0) : startScrollAnimation(props.animationDuration);
    };
